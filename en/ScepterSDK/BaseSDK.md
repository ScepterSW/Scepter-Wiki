...
1. For your convenience, the pre-compiled version of OpenCV 3.4.16 is already included in the Samples/OpenCV/Thirdparty directory. You do not need to download and compile it.
   ...
   The reason for choosing this version is that in ScepterSDK, OpenCV Sample is developed with OpenCV version 3.4.16. You can choose to use different versions of OpenCV according to your actual programming needs.

<!-- 2. 根据实际产品选择对应的 sample，以 NYX650 为例编译 OpenCV 显示例程 -->

2. Select the corresponding sample according to the actual product.
...
3. The FrameViewer.exe of the executable file generated by compilation is in the ScepterSDK\Windows\Bin\x86\ or ScepterSDK\Windows\Bin\x64\ directory.

<!-- 4. 运行 FrameViewer.exe，执行效果如下图。 -->

4. Run the FrameViewer.exe, and the execution effect is shown in the following figure.
