...
深度图视窗中显示数值为白点处实时像素点的深度值，单位为 mm，如上图该点深度值为 2899mm 。

IR 视窗中显示数值为白点处实时像素点的灰度值，如上图该点灰度值为 38 。

![WhitePoint](pic/WhitePoint.png)

> 鼠标右键单击可自行选择白点位置，同时显示对应点的深度值和灰度值。
...
Total Count：需要保存图像的数量，取值范围：\[1,100]。

Save：保存一或多帧当前所有显示区域的图像，点击右侧开关开始保存。如果显示区域未开启，则不会保存。
...
点击顶部菜单栏![设备固件升级](pic/Upgrade.png)，弹出设备固件升级设置页面。
