...
## 3.2. Device connection management
...
![设备管理](../../zh-cn/ScepterGUITool/pic/DeviceManage.png)
...
![图像显现](../../zh-cn/ScepterGUITool/pic/DisplayArea.png)

### 3.2.2. 设备信息

![设备信息](<../../zh-cn/ScepterGUITool/pic/Device information.png>)
...
![设备断开](../../zh-cn/ScepterGUITool/pic/DeviceStreamOff.png)
...
![显示区](../../zh-cn/ScepterGUITool/pic/DisplayArea.png)
...
深度图视窗中显示数值为白点处实时像素点的深度值，单位为 mm，如上图该点深度值为 2894mm 。

IR 视窗中显示数值为白点处实时像素点的灰度值，如上图该点灰度值为 39 。
...
Time Filter：时间滤波，降低图像帧间抖动。默认关闭，值越大，滤波效果越强。
...
RGB 传感器设置为自动曝光，下面会显示可以设置 ExposureTime(us)的滑块控制条。

当相机作业时会自动调节曝光时间。

ExposureTime(us) ：设置自动曝光的最大曝光时间，相机可以自动设置最大曝光时间。
