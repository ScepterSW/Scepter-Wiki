...
![Principle and Background](<Multi-Cameras-Coexist-asserts/01.png>)
...
DS86/87 相机的典型帧率为 15fps，即每秒可产生 15 帧的深度图像信息，所以其工作周期时间为 1s / 15 = 66.7ms，则每个工作周期的时间即为 66.7 毫秒。

细分每个工作周期，又可以分为曝光（激光发射与采集）、传输两个阶段，如下图：

![exposure and transmission](<Multi-Cameras-Coexist-asserts/02.png>)
...
基于 ToF 的原理和相机提供的从触发模式，DS 系列多机共存可通过协同控制方式来实现。
...
![Multi-Cameras Coexist](<Multi-Cameras-Coexist-asserts/03.png>)
...
![Hardware trigger](<Multi-Cameras-Coexist-asserts/04.png>)
