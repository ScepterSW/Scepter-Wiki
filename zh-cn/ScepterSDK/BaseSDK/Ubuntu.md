...
2. 根据实际产品选择对应的 sample，以 VENO86 产品编译 DeviceConnectByAlias 为例
   ...
   ```consle
   cd ScepterSDK/Ubuntu18.04/Samples/Base/VENO86
   mkdir build
   cd build/
   cmake ../
   make
...
3. 编译完成，输出路径为 PrecompiledSamples，进入目录后运行。
   ...
   ```consle
   cd ScepterSDK/Ubuntu18.04/PrecompiledSamples/VENO86_Samples/
...
2. 根据实际产品选择对应的 sample，以 VENO86 为例编译 OpenCV 显示例程

   ```consle
   cd ScepterSDK/Ubuntu18.04/Samples/OpenCV/VENO86
   make
   ```
